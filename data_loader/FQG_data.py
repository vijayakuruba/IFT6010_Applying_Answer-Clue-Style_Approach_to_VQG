...
from data_augmentor.FQG_data_augmentor import get_sample_probs
from .config import *
...
def sample_probs(debug):
    # prepro files
    CURRENT_PATH = os.getcwd().split("/")
    DATA_PATH = "/".join(CURRENT_PATH) + "/dataq/"
    # DATA_PATH = "../dataq/"
    ...
    return load(SAMPLE_PROBS_FILE_PATH)


def get_vqa_raw_examples(filename, debug=False, debug_length=20):
    """
    Get a list of raw examples given input VQA filename.
    """
    start = datetime.now()
    raw_examples = []

    SAMPLE_PROBS = sample_probs(debug)

    print("Start get VQA raw examples ...")
    ...
    with codecs.open(filename, "r", encoding="utf-8") as fh:
        ...
        for d in tqdm(data["data"]):
            ans_sent = d["context"]
            original_answer = d["answer_text"]
            question = d["question"]["question_text"]
            similar_answers, _, _, _ = FQG_data_augmentor.select_answers(ans_sent, SAMPLE_PROBS,
                                                                         original_answer=original_answer)
            answer_start = similar_answers[0][1]
            answer_text = similar_answers[0][0]
            example = {
                "question": question,
                "ans_sent": ans_sent,
...
def get_raw_examples(filename, filetype, debug=False, debug_length=20):
    ...
    elif filetype.lower() == "vqa":
        return get_vqa_raw_examples(filename, debug, debug_length)
    else:
...
def build_fqg_features(config, example, emb_dicts, related_words_dict):
    ...
    example["ans_sent_is_clue_hard"] = (example["ans_sent_is_overlap"] + example["ans_sent_is_content"] == 2.0).astype(
        float)
...
class QGData(Dataset):
    ...
    def __init__(self, config, emb_dicts, examples_file):
        ...
        for example in self.examples:
            ...
            example["switch_soft"] = (((example["switch_soft"] > 0).astype(float) +
                                       (example["switch_soft"] <= config.soft_copy_topN + 1).astype(
                                           float)) == 2.0).astype(float)
    ...
    def __getitem__(self, idx):
        return (self.examples[idx]["ans_sent_word_ids"],
                ...
                self.examples[idx]["ques_pos_ids"],
                # !!!!!! notice: we add <sos> <eos> to the ques, haven't handle them
