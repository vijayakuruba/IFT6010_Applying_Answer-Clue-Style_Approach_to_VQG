...
def select_similar_chunk(chunk_list, original_answer):
    ...
    for i in range(len(chunk_list)):
        ...
        if sim > top_sim:
            top_sim = sim
            chunk_id = i
    print("[DEBUG] check chunk_id:", chunk_id, "list len:", len(chunk_list))  # TO DELETE
    return chunk_list[chunk_id]


def select_most_similar_answer(chunklist, tree, doc, token2idx, idx2token, sentence, original_answer=None):
...
def select_answers(sentence, sample_probs,
    ...
    if original_answer is not None:
        return select_most_similar_answer(chunklist, tree, doc, token2idx, idx2token, sentence, original_answer)
