...
from .config import *
from common.constants import NLP, PARSER, FUNCTION_WORDS_LIST, QUESTION_TYPES, EXP_PLATFORM, SBERT_MODEL
...
def get_dataset_info(filename, filetype, save_file=None,
    ...
    examples_with_info = []
    print(f"\n[DEBUG] get_dataset_info(),   raw_examples: {len(raw_examples)}")
    ...
    print("[DEBUG] return examples_with_info")
...
def select_similar_chunk(chunklist, original_answer):
    if original_answer.lower() == "yes" or original_answer.lower() == "no":
        return None

    original_answer_vec = SBERT_MODEL.encode([original_answer])[0]
    MIN_SIM = 0.65
    INVALID_POS_TAGS = {"CC", "DT", "EX", "IN", "LS", "MD", "POS", "TO"}
    most_similar_chunks_ids = []
    for i in range(len(chunklist)):
        pos_tag = chunklist[i][1]
        answer = " ".join(chunklist[i][2])
        sim = cosine(original_answer_vec, SBERT_MODEL.encode([answer])[0])
        if sim >= MIN_SIM and answer.lower() != "a" and pos_tag not in INVALID_POS_TAGS:
            most_similar_chunks_ids.append(i)

    if len(most_similar_chunks_ids) == 0:
        return None
    chunk_id = most_similar_chunks_ids[np.random.randint(len(most_similar_chunks_ids))]
    return chunklist[chunk_id]


def select_most_similar_answer(sample_probs, chunklist, tree, doc, token2idx, idx2token, sentence,
                               original_answer=None):
    # select chunk most similar to original answer
    chunk = select_similar_chunk(chunklist, original_answer)
    # print(f"[DEBUG 0] chunk is {chunk is not None} --> {' '.join(chunk[2]) if chunk is not None else ''}")

    # If no similar chunk was found, pick the chunk with highest prob of being an answer
    # and that is not a determiner, or a preposition
    if chunk is None:
        a_probs = np.array(compute_answers_probs(sample_probs, chunklist))
        # Normalize probabilities
        a_probs = (a_probs - np.min(a_probs)) / (np.max(a_probs) - np.min(a_probs))
        best_chunks_ids = []
        min_prob = 0.80
        INVALID_POS_TAGS = {"CC", "DT", "EX", "IN", "LS", "MD", "POS", "TO"}
        while min_prob >= 0.0:
            for i in range(a_probs.shape[0]):
                if a_probs[i] < min_prob:
                    continue
                pos_tag = chunklist[i][1]
                answer = " ".join(chunklist[i][2])
                if (answer.lower() != "a" and pos_tag not in INVALID_POS_TAGS) or min_prob <= 0.05:
                    best_chunks_ids.append(i)

            if len(best_chunks_ids) > 0:
                break
            else:
                print(f"[DEBUG] Not found! {min_prob} ---> {min_prob*0.9}")
                best_chunks_ids = []
                min_prob *= 0.9

        chunk_id = best_chunks_ids[np.random.randint(len(best_chunks_ids))]
        chunk = chunklist[chunk_id]

    chunk_ner_tag, chunk_pos_tag, leaves, st_, ed_ = chunk
    sampled_answers = []
    ...
    try:
        ...
        print(f"Could not sample an answer similar to \"{original_answer}\" from the context")
    return sampled_answers, chunklist, tree, doc


def compute_answers_probs(sample_probs, chunklist, answer_length_bin_width=3, answer_length_min_val=0,
