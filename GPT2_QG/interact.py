...
def sample_sequence(inst, tokenizer, model, args, para_cache):
    ...
    instance, _ = build_acsq_only_input_from_segments(inst, tokenizer,
                                                      with_eos=False)  # as this is used for generate, with_eos is False.
...
def run():
    ...
    parser.add_argument("--calc_tg_metrics", action='store_true',
                        help="whether we need to calculate text generation metrics: bleu, etc.")
        ...
        try:
            with torch.no_grad():
                ...
                if para_index != para_cache["index"]:
                    ...
                    instance, _ = build_para_only_input_from_segments(inst,
                                                                      tokenizer)  # !!!! this is designed to re-use paragraph.
                    input_ids = torch.tensor(instance['input_ids'], device=args.device).unsqueeze(0)
                    token_type_ids = torch.tensor(instance['token_type_ids'], device=args.device).unsqueeze(0)
            ...
            if original_paragraph[
               output["answer_position"]:output["answer_position"] + len(original_answer)] != original_answer:
                # This should never be executed, only used as a last resort
                #print("Answer mismatch!")
                #print("answer by position: ",
                #      original_paragraph[output["answer_position"]:output["answer_position"] + len(original_answer)])
                ...
                if new_answer_position > 0:  # not -1
                    ...
                    original_ans_position = output["answer_position"]
                    #print("After revised answer position: ", original_paragraph[
                    #                                         output["answer_position"]:output["answer_position"] + len(
                    #                                             original_answer)])
                else:
                    continue
    ...
    with open(args.output_file, "w", encoding="utf-8") as f:
        json.dump(final_output_dict, f, indent=4, ensure_ascii=False)
        # f.write(json.dump(final_output_dict, indent=4, ensure_ascii=False).encode('utf8'))
