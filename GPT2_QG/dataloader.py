...
def get_processed_examples(raw_examples, debug=False, debug_length=20, shuffle=False):
    ...
    for e in tqdm(raw_examples):
        ...
        if len(clue_token_position) > 0 and clue_tokenized_text is not None:
            ...
            if len(
                    spans) > clue_end_token_idx:  # !!! NOTICE: sometimes we have len(spans) <= clue_end_token_idx. Need further debug...
...
def get_augmented_sents_examples(augmented_sentences_pkl_file, debug=False, debug_length=20, sent_limit=100,
                                 ans_limit=30):
    """
...
def get_dataset(tokenizer, dataset_cache, path, split='train', filetype='squad', debug=False, debug_length=20):
    ...
    data = get_positional_dataset_from_file(tokenizer, file=path, filetype=filetype, debug=debug,
...
def get_positional_dataset_from_file(tokenizer, file, filetype="squad", debug=False, debug_length=20):
    if filetype == "augmented_sents":
        ...
        data = get_raw_examples(file, filetype, debug,
                                debug_length)  # NOTICE: add handler for data augmented input data.
    ...
    for inst in tqdm(data):
        ...
        total_seq_len = len(tokenized_para) + len(tokenized_answer) + len(tokenized_question) + len(
            tokenized_clue) + len(tokenized_qtype) + 6
        ...
        if total_seq_len > tokenizer.max_len:
            ...
            assert len(tokenized_para) + len(tokenized_answer) + len(tokenized_question) + len(tokenized_clue) + len(
                tokenized_qtype) + 6 < tokenizer.max_len
